...
import socket
import logging
from plugins.mail_scan.utilities import parse_email_message
from plugins.read_pass.core import read_pass
...
"""
account: dict = {"email": "","password": ""} -> аккаунт сотрудника
last_date: str = '2025-10-13' -> сканировать письма от этой даты
last_timestamp: dt  = datetime(2025, 24, 13) -> как будто бы тоже самое. только применяется в другой функции
"""
...
class YandexMailScanner:
    ...
    last_activity = None
    keepalive_interval = 300  # 5 минут

    def __post_init__(self):
        self.email = self.account["email"]
        self.password = self.account["password"]
        self.search_criteria = self._create_params()

    def _create_params(self) -> str:
        search_criteria = ""

        if self.params.get("status", None):
            search_criteria += self.params["status"].keys()[0] + " "

        for param in ["dates", "addresses", "sizes"]:
            values = self.params.get(param, None)
            if values is not None:
                for key, value in values.items():
                    search_criteria += f'{key} "{value}" '

        if self.params.get("attachments", None):
            if self.params["attachments"]:
                search_criteria += "HEADER Content-Disposition attachment"

        search_criteria = search_criteria.strip()

        if len(search_criteria) == 0:
            raise ValueError("Длина строчки фильтра слишком короткая")
        ...
        for folder in folders_list:
            try:
                ...
                if not message_ids[0]:
                    print(f"В папке '{folder}' новых сообщений не найдено")
                    continue
                    ...
                    try:
                        ...
                        if msg_data and msg_data[0] and isinstance(msg_data[1], bytes):
                            try:
                                ...
                                print(f"Сообщение {num}: ошибка декодирования")
                                continue

                        else:
                            print(f"Неверные данные сообщения {num}")

                    except Exception as e:
                        print(f"Ошибка обработки сообщения {num}: {e}")
                        continue

            except Exception as e:
                print(f"Ошибка сканирования папки {folder}: {e}")
                continue
